...
#
# 실행:
#   python main.py                           # 수집 → 벡터화 (전체)
#   python main.py collect                   # 수집만
#   python main.py clean-collect             # 테이블 완전 초기화(id=1부터) 후 재수집
#   python main.py vectorize                 # 배경 제거 후 벡터화
#   python main.py vectorize-local images.zip  # 로컬 이미지(zip/폴더)로 벡터화
#   python main.py reset-embeddings          # 전체 embedding 컬럼 NULL 초기화
# =============================================================================
...
def run_collect(db):
    from pipeline.collector import NaverCollector
    from core.config import CATEGORY_QUERIES
...
def run_vectorize(db):
    from pipeline.vectorizer import ProductVectorizer
...
def main():
    ...
    try:
        ...
        if mode not in ("collect", "clean-collect", "vectorize", "vectorize-local", "all", "reset-embeddings"):
            ...
            print("사용법: python main.py [collect|clean-collect|vectorize|vectorize-local|all|reset-embeddings]")
